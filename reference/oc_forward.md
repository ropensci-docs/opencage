...
Forward geocoding from a character vector of location names to latitude
...
- proximity:
  ...
  A list of points of length one or `length(placename)`. A point is a
  named numeric vector of a latitude, longitude coordinate pair in
  decimal format. `proximity` provides OpenCage with a hint to bias
  ...
  helper. For example: `proximity = oc_points(51.9526, 7.6324)`. Default
  is `NULL`.
...
Depending on the `return` argument, `oc_forward` returns a list with
either

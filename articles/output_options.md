...
{opencage} contains two main types of main functions distinguished by
the manner in which the results are returned:

1.  The
    [`oc_forward_df()`](https://docs.ropensci.org/opencage/reference/oc_forward_df.md)/[`oc_reverse_df()`](https://docs.ropensci.org/opencage/reference/oc_reverse_df.md)
    functions always return a single tibble.
2.  The
    [`oc_forward()`](https://docs.ropensci.org/opencage/reference/oc_forward.md)/[`oc_reverse()`](https://docs.ropensci.org/opencage/reference/oc_reverse.md)
    functions return a list of various types, depending on the `return`
    value you specify. The possible `return` values are `df_list`,
    `json_list`, `geojson_list` and `url_only`.

Use of the
[`oc_forward_df()`](https://docs.ropensci.org/opencage/reference/oc_forward_df.md)/[`oc_reverse_df()`](https://docs.ropensci.org/opencage/reference/oc_reverse_df.md)
functions is demonstrated in the “Introduction to opencage” and the
“Customise your query” vignettes
([`vignette("opencage")`](https://docs.ropensci.org/opencage/articles/opencage.md)
and
[`vignette("customise_query")`](https://docs.ropensci.org/opencage/articles/customise_query.md),
respectively) . The function arguments mentioned in these other
vignettes are also generally available with
[`oc_forward()`](https://docs.ropensci.org/opencage/reference/oc_forward.md)/[`oc_reverse()`](https://docs.ropensci.org/opencage/reference/oc_reverse.md).
Here we will show the different return values available with
[`oc_forward()`](https://docs.ropensci.org/opencage/reference/oc_forward.md)/[`oc_reverse()`](https://docs.ropensci.org/opencage/reference/oc_reverse.md).
...
The `df_list` type drives the
[`oc_forward_df()`](https://docs.ropensci.org/opencage/reference/oc_forward_df.md)/[`oc_reverse_df()`](https://docs.ropensci.org/opencage/reference/oc_reverse_df.md)
functions. You can use the `df_list` output in a
[`dplyr::mutate()`](https://dplyr.tidyverse.org/reference/mutate.html)
chain to replicate the functionality of
...
This creates a list column `oc_result`, which can be easily unnested
with
...
OpenCage’s main output format is JSON. When you specify `json_list` as
the return type, you get the JSON as an R
...
When you choose `geojson_list` as the return type, the geocoder response
will be returned as GeoJSON specified as an R
[`list()`](https://rdrr.io/r/base/list.html).
...
In fact, {opencage} returns a list of results in `geo_list` format,
which should be compatible with the
{[geojsonio](https://docs.ropensci.org/geojsonio/)} package.
...
Your OpenCage API key is masked with the `OPENCAGE_KEY` string, by
default. If you really want {opencage} to display your API key with the
URL, set the `show_key` argument in
...
Note that the API key will only be returned with the URL in
[`base::interactive()`](https://rdrr.io/r/base/interactive.html) mode.

## `xml`

{opencage} does not support the XML response type at the moment. Please
file an [issue](https://github.com/ropensci/opencage/issues) or a
[pull-request](https://github.com/ropensci/opencage/pulls) if you have a
use-case that requires this.
...
have an `add_request` argument, indicating whether the request is
returned again with the results. If the `return` value is a `df_list`,
the `placename` or `latitude,longitude` is added as a column to the
results without a roundtrip to the API. `json_list` results will contain
all request parameters as returned by the API. This would normally
include your OpenCage API key, but {opencage} masks the key and replaces
it with the `OPENCAGE_KEY` string in the output. `add_request` is
currently ignored by OpenCage for GeoJSON results.
